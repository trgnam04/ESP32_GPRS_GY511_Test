...
void setup()
{
    ...
    xTaskCreatePinnedToCore(Task_ReadSensor, "Task_ReadSensor", 1024 * 5, NULL, 2, &TaskHandle_ReadSensor, 0);
    ...
    xTaskCreatePinnedToCore(Task_CheckConnection, "Task_CheckConnection", 1024 * 2, NULL, 1, &TaskHandle_CheckConnection, 1);
    // vTaskSuspend(TaskHandle_CheckConnection);

    xTaskCreatePinnedToCore(Task_SendData, "Task_SendData", 1024 * 3, NULL, 2, &TaskHandle_SendData, 1);
    // vTaskSuspend(TaskHandle_SendData);
    // xTaskCreatePinnedToCore(Task_MenuProcess, "Task_MenuProcess", 1024 * 2, NULL, 4, &TaskHandle_MenuProcess, 1);
    // xTaskCreatePinnedToCore(Task_ReadRotaryEncoder, "Task_ReadRotary", 1024, NULL, 3, &TaskHandle_ReadRotary, 1);
}
...
void convertData(void)
{
    ...
    snprintf(buffer, 128, "%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f",
             Sensor_Data.Ax, Sensor_Data.Ay, Sensor_Data.Az, Sensor_Data.lat, Sensor_Data.lng, Sensor_Data.AxFilter, Sensor_Data.AyFilter, Sensor_Data.AzFilter);
#endif
}
...
void displaySubPage(uint8_t idx)
{
    ...
    case 1:
    {
        ...
        params[3] = "tID";
        params[4] = "rID";
        dtostrf(Sensor_Data.AxFilter, 6, 2, values[0]);
        dtostrf(Sensor_Data.AyFilter, 6, 2, values[1]);
        dtostrf(Sensor_Data.AzFilter, 6, 2, values[2]);
...
void Task_ReadSensor(void *pvParameters)
{
    ...
    static uint16_t SensorSampleCount = 0;
    ...
    Sensor_Data.deltaT = float(taskInterval) / 1000.0;
        ...
        if (xSemaphoreTake(xI2CSemaphore, portMAX_DELAY))
        {
            ...
            while (1)
            {
                ...
                if (currentMillis - getDataMillis >= getDataInterval)
                {
                    ...
                    Sensor_Data.Az += eventAccel.acceleration.z  + 0.1;
                }

                if (currentMillis - taskMillis >= taskInterval)
                {
                    taskMillis = currentMillis;
                    Sensor_Data.Ax = Sensor_Data.Ax / (float)SensorSampleCount;
                    Sensor_Data.Ay = Sensor_Data.Ay / (float)SensorSampleCount;
                    Sensor_Data.Az = Sensor_Data.Az / (float)SensorSampleCount;

                    Sensor_Data.Gx = Sensor_Data.Gx / (float)SensorSampleCount;
                    Sensor_Data.Gy = Sensor_Data.Gy / (float)SensorSampleCount;
                    Sensor_Data.Gz = Sensor_Data.Gz / (float)SensorSampleCount;

                    Sensor_Data.Magx = Sensor_Data.Magx / (float)SensorSampleCount;
                    Sensor_Data.Magy = Sensor_Data.Magy / (float)SensorSampleCount;
                    Sensor_Data.Magz = Sensor_Data.Magz / (float)SensorSampleCount;
                    ...
                    vx += ax_filtered * Sensor_Data.deltaT;
                    vy += ay_filtered * Sensor_Data.deltaT;
